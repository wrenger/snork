...
pub fn max_n(game: &Game, depth: usize) -> [usize; 4] {
    let mut flood_fill = FloodFill::new(game.grid.width, game.grid.height);
    max_n_rec(&game, depth, 0, 0, [Direction::Up; 4], &mut flood_fill)
}
...
fn max_n_rec(
    ...
    if current_ply_depth == actions.len() {
        ...
        if current_depth + 1 >= depth {
            ...
            let mut result = max_n_rec(&game, depth, current_depth + 1, 0, actions, flood_fill);
            // max
            for i in 1..3 {
